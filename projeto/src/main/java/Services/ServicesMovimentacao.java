	...
	public void realizarMovimentacao(EntityManager em, List<Produtos> listaDeProdutos, Movimentacao mov) {
		...
		try {
			if (mov != null) {
				...
				NotaFiscal notaEmitida = serviceNotaFIscal.emitirNotaFiscal(em, listaDeProdutos, mov);
				mov.setNota_referenciada(notaEmitida);
				et.commit();
			}
		...
		try {
			if (movBuscada != null) {
				...
				if (validadorDeTempo > 10) {
					...
					serviceNotaFIscal.cancelarNotaComBaseMovimentacao(em, movBuscada);
					et.commit();
				}
			}
		} catch (Exception e) {
